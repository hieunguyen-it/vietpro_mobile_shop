...
$message = '';  
 $error = '';  
 if(isset($_POST["submit"]))  // Bắt đầu kiểm tra name"submit"
 {  
           if(file_exists('employee_data.json'))  // Kiểm tra xem file hoặc thư mục có tồn tại hay không.
           {  
                $current_data = file_get_contents('employee_data.json');  // đọc nội dung file định dạng json
                $array_data = json_decode($current_data, true);  //giải mã chuỗi đã mã hóa JSON
                // Khai báo mảng liên hợp thiết lập liên kết key và value .
                $extra = array(  
                     'name'               =>     $_POST['name'],  
                     'phone'          =>     $_POST["phone"],  
                     'mail'     =>     $_POST["mail"]  ,
                ...
                $array_data[] = $extra;  // gán giá trị của mảng vừa khai báo với chuỗi khai báo mảng đã được mã hóa
                $final_data = json_encode($array_data);  // Sử dụng hàm json_encode để convert giá trị string chỉ định thành định dạng JSON.
                // Kiểm tra nếu hàm hàm đó tồn tài thì ghi nội dung đó
...
<div id="my-cart">
        ...
        <form method="post">
            ...
            ?>
                <div class="cart-item row">
                    ...
                    <div class="cart-price col-lg-3 col-md-3 col-sm-12"><b><?php echo number_format($total_price); ?>đ</b><a href="module/cart/cart_dell.php?prd_id=<?php echo $row["prd_id"]; ?>">Xóa</a></div>
                </div>
            <?php
            }
            ?>
    ...
    ?>
        <div class="alert alert-danger"> Giỏ hàng của bạn hiện không có sản phẩm !</div>
    <?php
    }
    ?>
</div>
...
<?php
if (isset($_POST["name"]) && isset($_POST["mail"]) && isset($_POST["phone"]) && isset($_POST["add"]) && isset($_POST["submit"])) {
    ...
    while ($row = mysqli_fetch_array($query)) {
        ...
        $str_body .= '
            <tr>
                ...
                <td width="20%">' . number_format($total_price) . '</td>
            </tr> ';
    }
    ...
    $sql = "SELECT * FROM config_mail" ; // Viết câu truy vấn lựa chọn tất cả các cột trong config_mail
    $query = mysqli_query($conn , $sql) ; // Kết nối truy vấn đế CSDL
    $row = mysqli_fetch_array($query) ;// Hiện thị ra nội dưng được truyền vào biến $query
    ...
    try {
        ...
        //Recipients
        $mail->CharSet = 'UTF-8';
        $mail->setFrom(''.$row["mail_addcc"].'', 'Vietpro Mobile Shop');                // Gửi mail tới Mail Server
...
///////////////////////////////////code lưu đơn hàng vào file json////////////////////////////////////////////////////////////////////////
...
<div id="customer">
    <form method="post" id="buy-now">
        ...
        <div class="by-now col-lg-6 col-md-6 col-sm-12">
            <button onclick="buyNow()" id="mail_server" name="submit" type="submit" value="Append">Mua Ngay
                <br>
